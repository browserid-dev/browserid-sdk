 ...
 */
export const createKeypair = async ({ userId }: { userId: string }): Promise<GenerateKeysResponse> => {
    if (!userId?.trim()) {
    ...
    try {
        ...
        try {
            await storePrivateKeyOnDevice({ privateKey: keypair.privateKey, userId, deviceId, hostname });
        } catch (error) {
